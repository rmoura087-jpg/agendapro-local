...
export function AgendaPage({establishment}:{establishment:Establishment}) {
  ...
  return <div><div className="mb-6 flex flex-col gap-4 md:flex-row md:items-end md:justify-between"><div><div className="text-sm text-ink-400">Agenda</div><h1 className="text-3xl font-display font-medium">{dateLongBR(date)}</h1></div><button className="btn-brass" onClick={()=>setOpen(true)}><Plus size={17}/> Novo agendamento</button></div>
    ...
    <div className="card divide-y">{items.length===0?<div className="p-10 text-center text-sm text-ink-400">Nenhum horário agendado para este dia.</div>:items.map(a=><div className="flex flex-wrap items-center gap-4 p-4" key={a.id}><div className="w-16 font-display font-medium">{a.start_time.slice(0,5)}</div><div className="min-w-[180px] flex-1"><div className="font-bold">{a.customers?.name}</div><div className="text-sm text-ink-400">{a.services?.name} • {a.professionals?.name}</div></div><div className="font-semibold">{money(a.price)}</div><span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${a.status==="cancelled"?"bg-clay-50 text-clay-600":"bg-moss-50 text-moss-600"}`}>{a.status==="cancelled"?"Cancelado":"Agendado"}</span>{a.status!=="cancelled"&&<button className="text-sm font-semibold text-clay-500" onClick={()=>cancel(a.id)}>Cancelar</button>}</div>)}</div>
    {open&&<Modal title="Novo agendamento" onClose={()=>setOpen(false)}><form onSubmit={create} className="grid gap-4">
      ...
      <button className="btn-brass">Salvar agendamento</button>
    </form></Modal>}
  </div>;
}
