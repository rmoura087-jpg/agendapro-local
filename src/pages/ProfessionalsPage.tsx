...
export function ProfessionalsPage({establishment}:{establishment:Establishment}) {
  ...
  return <div><div className="mb-6 flex items-end justify-between"><div><div className="text-sm text-gray-500">Equipe</div><h1 className="text-3xl font-black">Profissionais</h1></div><button className="btn-primary" onClick={()=>start()}><Plus size={17}/> Novo profissional</button></div>
  {items.length===0?<EmptyState title="Nenhum profissional cadastrado" text="Cadastre quem realiza os serviços para liberar a agenda pública." action={<button className="btn-primary" onClick={()=>start()}>Cadastrar profissional</button>}/>:<div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">{items.map(p=><div className="card p-5" key={p.id}><div className="flex h-12 w-12 items-center justify-center rounded-full bg-gray-100 font-bold">{p.name.slice(0,1).toUpperCase()}</div><h3 className="mt-4 font-bold">{p.name}</h3><p className="text-sm text-gray-500">{p.specialty||"Profissional"}</p><p className="mt-1 text-xs text-gray-400">{p.phone}</p><div className="mt-5 flex gap-2"><button className="btn-secondary flex-1" onClick={()=>start(p)}><Pencil size={15}/> Editar</button><button className="btn-secondary px-3 text-red-600" onClick={()=>remove(p.id)}><Trash2 size={15}/></button></div></div>)}</div>}
  {open&&<Modal title={editing?"Editar profissional":"Novo profissional"} onClose={()=>setOpen(false)}><form onSubmit={save} className="grid gap-4"><div><label className="label">Nome</label><input className="input" value={form.name} onChange={e=>setForm({...form,name:e.target.value})} required/></div><div><label className="label">Especialidade</label><input className="input" value={form.specialty} onChange={e=>setForm({...form,specialty:e.target.value})} placeholder="Barbeiro, cabeleireiro..."/></div><div><label className="label">Telefone</label><input className="input" value={form.phone} onChange={e=>setForm({...form,phone:e.target.value})}/></div><button className="btn-primary">Salvar</button></form></Modal>}
  </div>
}
